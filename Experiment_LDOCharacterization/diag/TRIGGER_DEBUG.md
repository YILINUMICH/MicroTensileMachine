# Experiment_LDOCharacterization — Scope Trigger Debug Handoff
...
> - **Real fix (`../Driver_SiglentOscilloscope/oscilloscope.py`, shared driver):** the
...
/ scope = SDS2204X Plus (fw 5.4.1.5.2R2) @ 169.254.111.4:5025 / H7 = Firmware_SMADriver_PIO on COM8 /
...
3. **`WF? DAT2` block ends in a DOUBLED newline `\n\n`** — the binary-block
   ...
   `../Driver_SiglentOscilloscope/oscilloscope.py` `_query_block` (non-blocking drain).
...
| `../Driver_SiglentOscilloscope/oscilloscope.py` | shared driver; `_query_block` terminator-drain fix lives here |
| `../Firmware_SMADriver_PIO/src/main.cpp` | firmware: `fire <code_to> [ms] [code_from]`, `TRIG_PIN = PJ_11` |
