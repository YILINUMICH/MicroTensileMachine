...
> **Status: To-Test — dynamics verified (2026-06-17), absolute volts deferred.** Settling/overshoot/rise/ripple are bench-verified; `codes_per_div` (absolute-volt scaling) is still unconfirmed. See [STATUS.md](STATUS.md). One-off debug scripts are in [`diag/`](diag/). Plan: [`../docs/PLAN_phase6_ldo_characterization.md`](../docs/PLAN_phase6_ldo_characterization.md).

Automated time-domain characterization of the MCP4728 → TPS7A57 **DAC-margining**
LDO structure used by [`../Firmware_SMADriver_PIO/`](../Firmware_SMADriver_PIO/). Answers two
...
```
 H7 (Firmware_SMADriver_PIO)                     SDS2000X Plus
...
# 0. Flash the updated Firmware_SMADriver_PIO firmware (adds `fire`), power-cycle the rig.
...
`fire` logs now include `I_mA` and `R_ohm`. See [`../Firmware_SMADriver_PIO/README.md`](../Firmware_SMADriver_PIO/README.md).
...
- The trigger SCPI verbs and the `INR?`/`SAST?` completion poll were **debugged
  and confirmed on the bench** (2026-06-17); the debug history is in
  [`diag/TRIGGER_DEBUG.md`](diag/TRIGGER_DEBUG.md). Absolute-volt scaling
  (`codes_per_div`) is the remaining unverified item — see STATUS.md.
