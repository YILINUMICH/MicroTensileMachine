...
| *(LCR driver)*    | Provided by [`../Driver_KeysightLCR/lcr_meter.py`](../Driver_KeysightLCR/lcr_meter.py) — imported via `sys.path` shim in `workers.py`. No local copy. |
...
```bash
python ../Driver_KeysightLCR/test_lcr_meter.py --quick    # LCR connection check
