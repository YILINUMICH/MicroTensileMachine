...
| **Used by** | `SMA_CharacterizationV2/workers.py` imports `LCRMeter`, `MeasurementConfig`, `MeasurementFunction` directly via a `sys.path` shim — this module is the canonical LCR driver, no duplicates. `SMA_Characterization/` (v1, archived) still has its own legacy `lcr_reader.py` — left alone since the folder is frozen. |
| **Owner** | Yilin |
