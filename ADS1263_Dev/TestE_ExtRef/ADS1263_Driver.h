 ...
 * @brief ADS1263 32-bit ADC driver — Portenta H7 Elite + Hat Carrier (STABLE)
 *
 * Signal chain: GSO-50 load cell → INA818 (gain=200) → ADS1263
 * - INA818 provides all signal amplification (gain=200)
 * - ADS1263 PGA bypassed (gain=1), internal 2.5V reference
 * - Differential input: AIN0(+) vs AIN1(-)
 *
 * DRDY NOTE (confirmed via pull diagnostic):
 *   PJ_11 (Pi pin 11) is the correct DRDY pin — it IS connected to the
 *   ADS1263 DRDY signal through the Hat Carrier J5 routing. It must be
 *   configured as INPUT_PULLUP (not INPUT) to work correctly:
 *   - ADS1263 DRDY idle   → pin floats HIGH via internal pullup ✓
 *   - ADS1263 DRDY assert → chip drives LOW, overcomes pullup ✓
 *   No jumper wire required. The LoRa conflict theory was incorrect —
 *   the Portenta H7 Elite has no LoRa module and PJ_11 is free.
 */
...
// ── Pin definitions — Portenta H7 Elite + Hat Carrier (J5 Pi-compatible header) ──
// Waveshare ADS1263 HAT plugs directly onto J5.
// DRDY: use INPUT_PULLUP mode — pin floats HIGH when idle, ADS1263 drives LOW
// when conversion is ready. Confirmed working without any jumper wire.
#define ADS1263_CS_PIN    PE_6   // J2-53, SAI D0,  Pi pin 15 (BCM GPIO22)
#define ADS1263_DRDY_PIN  PJ_11  // J2-50, GPIO 2,  Pi pin 11 (BCM GPIO17) — INPUT_PULLUP required
...
public:
    ...
    // Fallback method — prefer readSingle() which uses DRDY.
    // poll_ms should be >= 1000/SPS (e.g. 5ms at 400SPS, 1ms at 2400SPS).
