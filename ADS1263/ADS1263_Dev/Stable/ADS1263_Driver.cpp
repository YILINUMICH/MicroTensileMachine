 ...
 *
 * Configured for: GSO-50 load cell → INA818 (gain=200) → ADS1263
 * - PGA bypassed (gain=1), internal 2.5V reference
 * - Differential input: AIN0(+) vs AIN1(-)
 *
 * Verified: Test A (DC accuracy), Test B (noise), Test C (AC capture)
 * DRDY works via INPUT_PULLUP on PJ_11 — no jumper wire required.
 */
...
bool ADS1263_Driver::begin(ADS1263_DataRate_t rate) {
  ...
  pinMode(ADS1263_DRDY_PIN, INPUT_PULLUP);  // REQUIRED: float=HIGH idle, ADS1263 drives LOW when ready
...
ADC_Reading ADS1263_Driver::readPoll(uint32_t poll_ms) {
  // Poll-mode read — does not use DRDY pin.
  // Fallback method; prefer readSingle() which uses DRDY hardware.
  // Starts a single conversion, waits poll_ms, reads result.
  ADC_Reading r = {};
  ...
  sendCommand(ADS1263_CMD_START1);
  delay(poll_ms);  // wait for conversion (2.5ms @ 400SPS, use 5ms)
...
void ADS1263_Driver::printConfig() {
  ...
  Serial.println();
  Serial.print(F("Input: AIN0(+) vs AIN1(-)"));
  Serial.println();
