 ...
 * @brief ADS1263 Verification — Test C: AC Signal Capture at Target Rate
 *
 * Section 2.5 of the test procedure.
 *
 * Setup:
 *   - Waveform generator: sine wave, 2.0 Vpp, 1.2V DC offset (0.2V to 2.2V)
 *   - Connect to AIN0. AIN1 to GND.
 *   - Oscilloscope CH1 in parallel for reference.
 *
 * Serial commands (115200 baud):
 *   'a'  — run Test C at 10 Hz  (4000 samples, 400 SPS)
 *   'b'  — run Test C at 50 Hz  (4000 samples, 400 SPS)
 *   'c'  — run Test C at 100 Hz (4000 samples, 400 SPS)
 *   'd'  — run Test C at 100 Hz (4000 samples, 1200 SPS)
 *   'r'  — single reading
 *   'h'  — help
 *
 * Output format (CSV for plotting):
 *   sample_index, timestamp_ms, voltage_V
 *
 * Pass criteria (Section 2.5):
 *   - Amplitude matches generator (2.0 Vpp ±20 mV)
 *   - 100 full cycles in 10 s at 400 SPS / 10 Hz
 *   - Waveform recognizable at 100 Hz (≥4 samples/cycle)
 *
 * Portenta H7 + Hat Carrier pin mapping:
 *   CS    = PE_6  (J2-53, Pi pin 15)
 *   DRDY  = PJ_11 (J2-50, Pi pin 11) — LoRa conflict, poll mode used
