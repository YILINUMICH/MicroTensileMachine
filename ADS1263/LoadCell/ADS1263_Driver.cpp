 ...
 *
 * Built from the Stable + TestE_ExtRef drivers:
 *   - STATUS + CRC framing on SPI (INTERFACE = 0x05, 6-byte read)
 *   - REFMUX defaults to AVDD/AVSS (0x08) for a 5V reference
 *   - codeToVoltage() scales against _vref_V (5.0 V default)
 *   - setRefMux() added so the reference can be changed at runtime
 *   - Pin map = Arduino Uno (CS=10, DRDY=2, RESET=9), DRDY uses INPUT_PULLUP
 *   - Auto-cal at begin() is skipped (SFOCAL1 corrupts OFCAL if input != 0V);
 *     call calibrate() explicitly after shorting AIN0 to AIN1.
 */
...
ADS1263_Driver::ADS1263_Driver()
    : _spi(2000000, MSBFIRST, SPI_MODE1),  // 2 MHz, mode 1 (CPOL=0, CPHA=1)
      _rate(ADS1263_20SPS),
      _continuous(false),
      _refmux(ADS1263_REFMUX_AVDD_AVSS),  // external 5V ref (AVDD/AVSS)
      _vref_V(ADS1263_VREF_V)             // 5.0 V
{
}
...
bool ADS1263_Driver::begin(ADS1263_DataRate_t rate) {
    ...
    pinMode(ADS1263_DRDY_PIN, INPUT_PULLUP);  // idle HIGH via pullup, ADS1263 drives LOW when ready
    ...
    // POWER (0x01): Enable internal reference (harmless; still allows external via REFMUX)
    // Bit4 = VBIAS disabled, Bit0 = INTREF enabled
    writeRegister(ADS1263_REG_POWER, 0x11);
    delay(150);

    // INTERFACE (0x02): STATUS byte prepend + CRC enabled (0x05)
    // readRawData() expects STATUS + 4 data + CRC = 6 bytes.
    writeRegister(ADS1263_REG_INTERFACE, 0x05);

    // MODE0 (0x03): No chop, no delay
    writeRegister(ADS1263_REG_MODE0, 0x00);

    // MODE1 (0x04): Sinc3 filter, no simultaneous ADC2
    writeRegister(ADS1263_REG_MODE1, 0x40);

    // MODE2 (0x05): PGA bypassed (bit7=1), gain=1, data rate in DR[3:0]
    writeRegister(ADS1263_REG_MODE2, 0x80 | (_rate & 0x0F));

    // INPMUX (0x06): AIN0 positive, AIN1 negative (0x01)
    // AIN0 = LCA Vo, AIN1 = LCA GND (tie to analog ground)
    writeRegister(ADS1263_REG_INPMUX, 0x01);

    // REFMUX (0x0F): default is AVDD/AVSS (0x08) → external 5V reference
    ...
    // Clear offset cal registers (factory zero). SFOCAL1 is NOT run here —
    // it would corrupt OFCAL if AIN0/AIN1 are not actually shorted.
...
bool ADS1263_Driver::calibrate() {
    ...
    // SFOCAL1 may reset critical registers — re-apply them
    delay(100);
    writeRegister(ADS1263_REG_INTERFACE, 0x05);
    writeRegister(ADS1263_REG_MODE1, 0x40);
    writeRegister(ADS1263_REG_MODE2, 0x80 | (_rate & 0x0F));
...
int32_t ADS1263_Driver::readRawData() {
    ...
    // INTERFACE=0x05 → STATUS + 4 data + CRC = 6 bytes
    SPI.beginTransaction(_spi);
    digitalWrite(ADS1263_CS_PIN, LOW);
    delayMicroseconds(5);
    ...
    buf[0] = SPI.transfer(0xFF);  // MSB
    buf[1] = SPI.transfer(0xFF);
    buf[2] = SPI.transfer(0xFF);
    buf[3] = SPI.transfer(0xFF);  // LSB
