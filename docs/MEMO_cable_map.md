...
- [ ] **USB cables.** Which physical USB-A port on the host PC each instrument uses. **Current host (Yilin's Windows machine, 2026-05-25):**
    ...
    - **COM8** — Portenta H7 (Arduino CDC, USB VID:PID = `2341:025B` in normal mode; `2341:035B` in DFU mode). **Port history:** COM8 until 2026-07-24, when the original H7 was replaced (its M7 flash was left partially written by a DFU download failure) and the replacement enumerated on COM13. On **2026-07-27 the rig switched back to the original board**, which enumerates on COM8 again (serial `003B002A3033510B34323437`, confirmed from live `pio device list`); every `platformio.ini` and module `config.yaml` was repointed back. An older session log or STATUS entry naming either COM8 or COM13 means this same H7 role — check the date against the swaps above to tell which physical board produced it.
    - `Firmware_SMASensorHub_PIO/platformio.ini` pins `upload_port = COM8` and `monitor_port = COM8` (in a shared `[env]` block) so PIO doesn't grab the Zaber by COM-number race. COM-numbers are Windows-specific and may renumber when the H7 is replugged into a different USB-A port — if that happens, check `pio device list`, update the .ini, or override at the CLI with `--upload-port COMx`. The VID:PIDs above are stable across hosts; PIO's `upload_port` is a glob over port paths and doesn't accept VID:PID directly.
