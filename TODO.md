...
- [ ] **Port firmware from Hat Carrier to Mid Carrier (ASX00055).** Hardware has moved; firmware is partially ported.
    - [x] ~~`SensorHub_PIO/` — code port done 2026-05-25.~~ Pin defines (`PA_8`/`PC_6`/`PC_7`), REFMUX (`0x09`), POWER (`0x13` for VBIAS), ADC1 on AIN2/3 + ADC2 on AIN4/5, REF7050 shared on AIN0/1 — all in `lib/ADS1263/ADS1263_Driver.h` and `src/main.cpp`. **Bench-verify on Mid Carrier + bare TI EVM still pending** (see `SensorHub_PIO/STATUS.md`).
    - [ ] `LaserHead_PIO/` — still on Hat Carrier pin defines.
    - [ ] `LoadCell_PIO/` — still on Hat Carrier pin defines.
