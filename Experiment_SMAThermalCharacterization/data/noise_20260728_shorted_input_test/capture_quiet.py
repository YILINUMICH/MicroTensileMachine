...
try:
    ...
    pass
time.sleep(0.3)
# FORCE PULL: a full CDC TX buffer makes the port look dead. reset_input_buffer()
# clears the HOST side only; big raw reads are what give the firmware room again.
_t = time.time()
while time.time() - _t < 1.0:
    ser.read(65536)
ser.reset_input_buffer()

# Refuse to burn 60 s on a dead port — verify bytes are flowing first.
probe, _t = 0, time.time()
while time.time() - _t < 2.0 and probe < 2000:
    probe += len(ser.read(65536))
if probe < 2000:
    sys.exit(f"ABORT: only {probe} bytes in 2 s — the port is not streaming. "
             f"Power-cycle USB + EVM and retry.")
print(f"port live ({probe} bytes in the 2 s probe)")
