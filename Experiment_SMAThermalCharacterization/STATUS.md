...
- [ ] **LASER 65.8 Hz TONE — root-cause it** (found 2026-07-13; reference sample `data/console_20260713_122906`, diagnose with `plot_laser.py`). The laser's apparent ±1.4 µm "noise" is really a coherent **65.77 Hz / 1.72 µm** ripple carrying **74% of the channel's variance**; removing it takes σ from 1.41 → 0.72 µm (~4× better in the post-processing demo). **Load/ADC2 is clean at that frequency** and both ADCs share the REF7050, so it is specific to the **ADC1/laser path** (IL-030 analog output or AIN4/AIN5 wiring), not a common-mode supply artifact. **Do this first:** re-record idle at a **higher ADC data rate (1200/2400 SPS)** — at the current 400 SPS (Nyquist 200 Hz) the tone is equally consistent with a real **335 Hz or 466 Hz** source, and 65.8 vs 335 Hz implicate very different culprits. This currently sets the rig's displacement resolution: in an actuation run the real SMA displacement sits *below* this band.
