/**
 * @file main.cpp  (Portenta H7 - Firmware_SMADriver_PIO, M7-only)
 *
 ...
 *  Firmware_SensorHub_PIO M4's PA_8 / PC_6 / PC_7. The two firmwares can
 ...
 * (it has no M4). Once the SMA logic merges into Firmware_SensorHub_PIO M7,
...
static void cmdInfo() {
    Serial.println(F("\n== Firmware_SMADriver_PIO state (analytical model) =="));
...
void setup() {
    ...
    Serial.println();
    Serial.println(F("== Firmware_SMADriver_PIO  (Portenta H7 M7, Phase 6 bring-up) =="));
