"""
operator_io.py — terminal I/O for the SMA recorder session (V3).
...
class PhaseProgress:
    ...
    Use:
        ...
        while recording:
            prog.update(elapsed, lcr_n, h7_n, stage_n)
        prog.finalize(lcr_n, h7_n, duration_s, stage_n)
    """
    phase_name: str
    ...
    def update(self, elapsed_s: float, lcr_n: int, h7_n: int,
               stage_n: int = 0, *, force: bool = False) -> None:
        now = time.monotonic()
        ...
        stage_part = f"STG {stage_n:5d}" if stage_n else ""

        line = (f"  {self.phase_name.upper():<6} {time_part}  "
                f"{lcr_part}  {h7_part}  {stage_part}").rstrip()
    ...
    def finalize(self, lcr_n: int, h7_n: int, duration_s: float,
                 stage_n: int = 0) -> None:
        if self._started:
            sys.stdout.write("\r" + " " * self._line_width + "\r")
        extra = f" / {stage_n} stage" if stage_n else ""
        msg = (f"  {self.phase_name.upper():<6} complete — "
               f"{lcr_n} LCR / {h7_n} H7{extra} samples in {duration_s:.2f} s")
        print(msg)
        sys.stdout.flush()
...
def banner_session_start(session_id: str, output_dir: str) -> None:
    print()
    print("=" * 60)
    print(f"  SMA characterization session (V3): {session_id}")
...
def banner_health(lcr_pass: bool, lcr_n: int,
                  ...
                  timeout_s: float,
                  stage_pass: Optional[bool] = None, stage_n: int = 0) -> None:
    print()
    print("─" * 60)
    ...
    if all_pass:
        ...
        if stage_pass is not None:
            print(f"      STG: {stage_n} samples ✓")
    else:
        print(f"  ✘ Health check FAILED ({timeout_s:.1f} s timeout):")
        print(f"      LCR: {lcr_n} samples  {mark(lcr_pass)}"
              + ("" if lcr_pass else "  (check VISA / power / USB)"))
        print(f"      H7 : {h7_n} samples  {mark(h7_pass)}"
              + ("" if h7_pass else "  (check serial port / H7 power / firmware)"))
        if stage_pass is not None:
            print(f"      STG: {stage_n} samples  {mark(stage_pass)}"
                  + ("" if stage_pass else "  (check COM5 / Zaber power)"))
    print("─" * 60)
    sys.stdout.flush()
