...
> Author: Yilin Ma
> Last major update: 2026-08-03 — `Experiment_SMAThermalCharacterization/` reorganized into four buckets: module root (live rig code), `analysis/` (the standing raw→table→charts pipeline), `diagnostics/` (closed one-off investigations), and `data/` split into `raw/` (what the rig wrote) + `derived/` (what the pipeline computed). Capture folder names unchanged; pipeline verified byte-identical.
> 2026-07-14 — README reconciled with the on-disk tree: the sensing + SMA-drive firmware merged into `Firmware_SMASensorHub_PIO/` (former `Firmware_SensorHub_PIO/` + `Firmware_SMADriver_PIO/` retired); added `Firmware_SMARateTest_PIO/` (SMA stream-rate diagnostic), `Firmware_stable/` (frozen baseline), `Experiment_SMACharacterizationV3/`, and `Experiment_SMAThermalCharacterization/`; the SMA recorder moved V2 → V3; `TODO.md` stub removed.
...
| [`Experiment_SMAThermalCharacterization/`](Experiment_SMAThermalCharacterization/STATUS.md) | **WIP / To-Test** (fork of V3; not yet bench-run) | **SMA thermal characterization console** — a fork of V3 focused on correlating SMA temperature with the Joule-heating drive. Adds an adaptive-FPS camera + live preview; **LCR removed** from this module. Planned: a temperature stream (thermocouple / IR).<br>**Four-bucket layout (2026-08-03):** module root = live rig code (`operator_console.py` + 4 standing tools + 7 `lib_`); `analysis/` = the standing raw→table→charts pipeline; `diagnostics/` = closed one-off investigations; `data/raw/` = captures as the rig wrote them, `data/derived/` = merged tables + figures, all committed so results travel with a clone. |
...
| Run an SMA **thermal** characterization experiment | [`Experiment_SMAThermalCharacterization/`](Experiment_SMAThermalCharacterization/) — `python operator_console.py` (interactive analysis: `operator_explore.ipynb`) |
| Re-run the SMA thermal **analysis pipeline** (raw → per-cycle table → charts) | [`Experiment_SMAThermalCharacterization/analysis/`](Experiment_SMAThermalCharacterization/analysis/) — `python analyze_raw.py` then `plot_envelope.py` / `plot_energy.py` / `plot_selfsensing.py` / `plot_transition.py` / `plot_r_bias.py` / `plot_trajectory.py` |
| Find an SMA thermal capture vs. an analysed result | captures: `Experiment_SMAThermalCharacterization/data/raw/` · tables + figures: [`…/data/derived/`](Experiment_SMAThermalCharacterization/data/derived/) |
...
- **Separate raw captures from derived results** — the layout `Experiment_SMAThermalCharacterization/` established on 2026-08-03, and the one to follow when a module's `data/` outgrows a flat folder: `data/raw/` is what the instrument wrote (never hand-edited), `data/derived/` is what analysis computed (regenerable, but committed so results travel with a clone). Analysis code lives *outside* `data/` — in that module, `analysis/` for the standing pipeline and `diagnostics/` for closed one-off investigations. The other modules' `data/` folders are still flat; convert one when it starts mixing code, captures, and outputs, not before.
- **Session data is tracked on purpose.** Experiment captures under `*/data/` are committed (see `.gitignore`) so results are available on every machine. Only genuinely machine-local files are ignored: `__pycache__/`, `.venv/`, `.pio/`, and `zaber_config.json` (regenerated per host on every `ZaberStage.connect()`).
...
- **COM8 = Portenta H7, COM5 = Zaber stage.** `upload_port`/`monitor_port` are pinned to COM8 in every `platformio.ini` so PIO doesn't auto-pick the Zaber and wedge the stage. On a different host run `pio device list` and edit the `.ini` or override with `--upload-port COMx`. Port history: COM8 → COM13 at the 2026-07-24 board swap → COM8 again on 2026-07-27 when the rig switched back to the original board. Either number in older logs and data means this same port.
