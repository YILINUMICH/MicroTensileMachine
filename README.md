...
- **COM13 = Portenta H7, COM5 = Zaber stage.** `upload_port`/`monitor_port` are pinned to COM13 in every `platformio.ini` so PIO doesn't auto-pick the Zaber and wedge the stage. On a different host run `pio device list` and edit the `.ini` or override with `--upload-port COMx`. Was COM8 until the board swap on 2026-07-24 — older logs and data saying COM8 mean this same port.
